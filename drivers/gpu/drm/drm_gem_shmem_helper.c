...
{
	struct drm_gem_object *obj = &shmem->base;
	struct drm_device *dev __unused = obj->dev;
...
#elif defined(__FreeBSD__)
		/*
		 * The _locked suffix is a contract: the caller must hold
		 * the vm object lock across the insert (see the ttm
		 * fault handler, ttm_bo_vm.c).
		 */
...
int drm_gem_shmem_mmap(struct drm_gem_shmem_object *shmem, struct vm_area_struct *vma)
{
	...
	if (obj->import_attach) {
		...
		return ret;
#elif defined(__FreeBSD__)
		/* mmap of PRIME-imported buffers: not supported */
		return -ENOSYS;
#endif
	}
...
#elif defined(__FreeBSD__)
	/*
	 * No device-level DMA mapping: the LinuxKPI pci_dev bridge carries
	 * no DMA state (linux_pdev_dma_init is private to the LinuxKPI PCI
	 * attach path), and this port's only consumer (virtio-gpu with the
	 * DMA quirk, i.e. no ACCESS_PLATFORM) addresses pages physically
	 * via sg_phys()/page pointers, never sg_dma_address().
	 */
#endif
